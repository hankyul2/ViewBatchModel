...
def log_accs(args, logger, accs, t, setting, epoch=None, prefix="RESULT"):
    ...
    if not args.nowand:
        ...
        if setting == 'domain-il' or setting == 'general-continual':
            d2 = {f'{prefix}_mean_accs{postfix}': mean_acc,
                  **{f'{prefix}_acc_{i}{postfix}': a for i, a in enumerate(accs[0])},'Task': t}
...
class Logger:
    ...
    def log_system_stats(self, cpu_res, gpu_res):
        ...
        if not self.args.nowand:
            wandb.log({'CPU_memory_usage': cpu_res, 'GPU_memory_usage': gpu_res})
