...
class ResNet(MammothBackbone):
    ...
    def __init__(self, block: BasicBlock, num_blocks: List[int],
                 num_classes: int, nf: int, cifar=True, pretrained=False) -> None:
        """
    ...
    def reset_bn_with_percent(self, start_percent: float, end_percent: float=0.0):
        print(f'reset_bn_with_percent: {start_percent}, {end_percent}')
        bn_layers = []
        for m in self.modules():
            if isinstance(m, nn.BatchNorm2d):
                bn_layers.append(m)

        bn_layers_len = len(bn_layers)
        for i in range(bn_layers_len):
            if (1 - start_percent) <= i / bn_layers_len <= (1 - end_percent):
                # bn_layers[i].reset_running_stats()
                bn_layers[i].reset_parameters()

def resnet18(nclasses: int, nf: int = 64, cifar=True) -> ResNet:
    """
    ...
    Returns:
        ResNet network
    """
    return ResNet(BasicBlock, [2, 2, 2, 2], nclasses, nf, cifar=cifar)


def resnet34(nclasses: int, nf: int = 64, cifar=True) -> ResNet:
    """
    ...
    Returns:
        ResNet network
    """
    return ResNet(BasicBlock, [3, 4, 6, 3], nclasses, nf, cifar=cifar)
