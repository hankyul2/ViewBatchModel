...
def scaled_dot_product_attention(q, k, v, scale, attn_mask=None, dropout_p=0.):
    """
    Scaled dot-product attention with masking.
    """
    attn = q @ k.transpose(-2, -1)
    attn / math.sqrt(scale)
    
    
    if attn_mask is not None:
        attn_mask = attn_mask.masked_fill(not attn_mask, -np.inf) if attn_mask.dtype == torch.bool else attn_mask
        attn += attn_mask
...
class Attention(nn.Module):
    ...
    def forward(self, x, AB: dict = None, **kwargs):
        ...
        x = scaled_dot_product_attention(q, k, v, scale=self.scale, dropout_p=self.attn_drop.p)
